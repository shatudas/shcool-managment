...
class RegistrationFeeController extends Controller
{
    public function view()
   {
    ...
    return view('backend.student.registration_fee.view-registration_fee',$data);
   }
   ...
   $html['thsource'] .= '<th>Regstration Fee</th>';
   $html['thsource'] .= '<th>Discount Amount</th>';
   $html['thsource'] .= '<th>Fee(this Student)</th>';
   $html['thsource'] .= '<th>Action</th>';

   foreach ($allStudent as $key => $v) {
   	$registrationfee = FeeCategoryAmount::where('fee_category_id','1')->where('class',$v->class_id)->first();
   	$color = 'success';
    ...
    $html[$key]['tdsource'] .= '<a class="btn btn-sm btn-'.$color.'" title="Payslip" target="_blank" href="'.route("student.reg.fee.payslif").'?class_id='.$v->class_id.'&student_id='.$v->student_id.'">Fee Slip</a>';
    $html[$key]['tdsource'] .= '</td>';
